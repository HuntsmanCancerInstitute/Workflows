...
# Low Allele Frequency, High Read Depth, Tumor - Normal Somatic Variant Calling Workflow
...
# It utilized Bcftools to generate nearly every possible observed variant, filters the calls with a USeq tool called SimpleSomaticCaller, combines them with calls from Illumina's Manta and Strelka tools, and lastly z-scores the variants using a panel of high depth normals. See the USeq VCFBkz and BamPileup apps for details.
