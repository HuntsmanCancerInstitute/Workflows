...
# 5) Soft link or move your Foundation bam and bai index into the job dir 

# 6) Copy over the two workflow docs: xxx.README.sh and xxx.sm into the job directory.

# 7) Launch the xxx.README.sh via sbatch or run it on your local server.  
...
regionsForReadCoverage=$dataBundle/Bed/Foundation/foundationOne_CDxHemeLiquid_CCDS_SharedCodingExons_9Dec2020.bed.gz \
regionsForOnTarget=$dataBundle/Bed/Foundation/hg38FoundationMergedCdxHemeOnePad50bp.bed.gz \
indexFasta=$dataBundle/Indexes/B38IndexForBwa-0.7.17/hs38DH.fa \
indexFastaB37=$dataBundle/Indexes/B37/human_g1k_v37_decoy_phiXAdaptr.fasta \
